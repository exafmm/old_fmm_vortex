...
void jpbox(int lbj, int& lbjp, int lev, int ipb, int npb) {
  int ncmax,nmin,nmax,ntra,jc,jj,i,k,ja,na,jb,nb,jx,jy,jz;
  ...
  for( jj=0; jj<lbj; jj++ ) {
    ...
    if( ntra == 1 ) {
      ...
      if( nd[1]*nd[2] != 0 ) jpsub(0,nd[1],nd[2],jj,jc);
      if( nd[2]*nd[0] != 0 ) jpsub(nd[0],0,nd[2],jj,jc);
