...
void stl2p(int nmp, int mp, int lbi, double rb) {
  ...
  double rsp,xic,yic,zic,gxdd,gydd,gzdd,gxp[mjmax],gyp[mjmax],gzp[mjmax];
  M3_UNIT *n_unit;
  M3_CELL cell[nbnp];
  ...
  for( icall=0; icall<ncall; icall++ ) {
    ...
    n_unit = m3_allocate_unit("../../dat/bspp.tblmd3",M3_POTENTIAL,xmins,xmaxs,0);
    ...
    m3_set_charges(n_unit,gyp,nmd);
    m3_setup_overlap(n_unit);
    for( ii=jstamd[icall]; ii<=jendmd[icall]; ii++ ) {
      m3_set_cells(n_unit,&cell[ii],njsize);
      m3_calculate_potentials(n_unit,pos+ibase[ii],isize[ii],bymd+ibase[ii]);
    }
    m3_start_overlap_calculation(n_unit);
    m3_wait_overlap_calculation(n_unit);
    m3_set_charges(n_unit,gzp,nmd);
    m3_setup_overlap(n_unit);
    for( ii=jstamd[icall]; ii<=jendmd[icall]; ii++ ) {
      m3_set_cells(n_unit,&cell[ii],njsize);
      m3_calculate_potentials(n_unit,pos+ibase[ii],isize[ii],bzmd+ibase[ii]);
    }
    m3_start_overlap_calculation(n_unit);
    m3_wait_overlap_calculation(n_unit);
    nmdd = 0;
