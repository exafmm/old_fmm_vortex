  ...
  for( ii=0; ii<lbi; ii++ ) {
    ...
    for( i=ndi[0][ii]; i<=ndi[1][ii]; i++ ) {
      for( j=0; j<nmp; j++ ) {
        ...
        cutoff = gxi[i]*(ggy[j]*ziic-ggz[j]*yiic)+gyi[i]*(ggz[j]*xiic-ggx[j]*ziic)+gzi[i]*(ggx[j]*yiic-ggy[j]*xiic);
        gxd[i] -= 0.25/pi*(gyi[i]*ggz[j]-gzi[i]*ggy[j])/pow(r,3)-0.75/pi*cutoff*xiic/pow(r,5);
        gyd[i] -= 0.25/pi*(gzi[i]*ggx[j]-gxi[i]*ggz[j])/pow(r,3)-0.75/pi*cutoff*yiic/pow(r,5);
        gzd[i] -= 0.25/pi*(gxi[i]*ggy[j]-gyi[i]*ggx[j])/pow(r,3)-0.75/pi*cutoff*ziic/pow(r,5);
      }
    }
  }
}
