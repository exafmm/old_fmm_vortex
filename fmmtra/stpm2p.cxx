    ...
    for( ij=0; ij<nij[ii]; ij++ ) {
      ...
      for( i=ndi[0][ii]; i<=ndi[1][ii]; i++ ) {
        for( j=0; j<nmp; j++ ) {
          ...
          gxd[i] += 0.25/pi*(gyi[i]*ggz[j]-gzi[i]*ggy[j])/pow(r,3)+0.75/pi*(gxi[i]*xijc+gyi[i]*yijc+gzi[i]*zijc)*(ggy[j]*zijc-ggz[j]*yijc)/pow(r,5);
          gyd[i] += 0.25/pi*(gzi[i]*ggx[j]-gxi[i]*ggz[j])/pow(r,3)+0.75/pi*(gxi[i]*xijc+gyi[i]*yijc+gzi[i]*zijc)*(ggz[j]*xijc-ggx[j]*zijc)/pow(r,5);
          gzd[i] += 0.25/pi*(gxi[i]*ggy[j]-gyi[i]*ggx[j])/pow(r,3)+0.75/pi*(gxi[i]*xijc+gyi[i]*yijc+gzi[i]*zijc)*(ggx[j]*yijc-ggy[j]*xijc)/pow(r,5);
        }
      }
    }
  }
