...
void stm2p(int nmp, int mp, int lbi, int lbj, int lev, int ipb, double rb) {
  ...
  double rsp,xjc,yjc,zjc,xijc,yijc,zijc,r,cutoff;
    ...
    for( ij=0; ij<nij[ii]; ij++ ) {
      ...
      for( i=ndi[0][ii]; i<=ndi[1][ii]; i++ ) {
        for( j=0; j<nmp; j++ ) {
          ...
          cutoff = gxi[i]*(ggy[j]*zijc-ggz[j]*yijc)+gyi[i]*(ggz[j]*xijc-ggx[j]*zijc)+gzi[i]*(ggx[j]*yijc-ggy[j]*xijc);
          gxd[i] -= 0.25/pi*(gyi[i]*ggz[j]-gzi[i]*ggy[j])/pow(r,3)-0.75/pi*cutoff*xijc/pow(r,5);
          gyd[i] -= 0.25/pi*(gzi[i]*ggx[j]-gxi[i]*ggz[j])/pow(r,3)-0.75/pi*cutoff*yijc/pow(r,5);
          gzd[i] -= 0.25/pi*(gxi[i]*ggy[j]-gyi[i]*ggx[j])/pow(r,3)-0.75/pi*cutoff*zijc/pow(r,5);
        }
      }
    }
  }
