...
void dird(int n0, int n1, int n2, int n3) {
  ...
  for( icall=0; icall<nicall; icall++ ) {
    ...
    for( jcall=0; jcall<njcall; jcall++ ) {
      ...
      n_unit = m3_allocate_unit("../../dat/bsgf.tblmd3",M3_POTENTIAL,xminn,xmaxn,0);
      ...
      n_unit = m3_allocate_unit("../../dat/stgf.tblmd3",M3_FORCE,xminn,xmaxn,0);
      m3_set_positions(n_unit,pos,nmd);
      ...
      for( i=ista; i<=iend; i++ ) {
        nmd = i-ista;
        gxd[i] += 0.75/pi*gxi[i]*(yi[i]*zmd[nmd][0]-zi[i]*ymd[nmd][0]-xmd[nmd][0]);
        gyd[i] += 0.75/pi*gxi[i]*(yi[i]*zmd[nmd][1]-zi[i]*ymd[nmd][1]-xmd[nmd][1]);
        gzd[i] += 0.75/pi*gxi[i]*(yi[i]*zmd[nmd][2]-zi[i]*ymd[nmd][2]-xmd[nmd][2]);
      }
      ...
      for( i=ista; i<=iend; i++ ) {
        nmd = i-ista;
        gxd[i] += 0.75/pi*gyi[i]*(zi[i]*xmd[nmd][0]-xi[i]*zmd[nmd][0]-ymd[nmd][0]);
        gyd[i] += 0.75/pi*gyi[i]*(zi[i]*xmd[nmd][1]-xi[i]*zmd[nmd][1]-ymd[nmd][1]);
        gzd[i] += 0.75/pi*gyi[i]*(zi[i]*xmd[nmd][2]-xi[i]*zmd[nmd][2]-ymd[nmd][2]);
      }
      ...
      for( i=ista; i<=iend; i++ ) {
        nmd = i-ista;
        gxd[i] += 0.75/pi*gzi[i]*(xi[i]*ymd[nmd][0]-yi[i]*xmd[nmd][0]-zmd[nmd][0]);
        gyd[i] += 0.75/pi*gzi[i]*(xi[i]*ymd[nmd][1]-yi[i]*xmd[nmd][1]-zmd[nmd][1]);
        gzd[i] += 0.75/pi*gzi[i]*(xi[i]*ymd[nmd][2]-yi[i]*xmd[nmd][2]-zmd[nmd][2]);
      }
