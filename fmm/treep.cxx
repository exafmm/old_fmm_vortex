...
extern void ipbox(int, int, int&, int, int, int, int&, int&, int, int, int, double*);
...
extern void bsm2p(int, int, int, int, int, int, double);
extern void stm2p(int, int, int, int, int, int, double);
extern void potm2p(int, int, int, int, int, int, double);
extern void unsorti(int&);
extern void unsortj(int&);

void fmm(int mi, int mj, int neqd, double* tfmm){
  int neq,lev,lmin,nebmp,nbnep,i,nmp,lbi,nbyte,ii,lbj,j,lbjp,ic,jj,mjp,lbjr,lbjo,ib,ipb;
...
// Step 1.1 Send and recv particles on other nodes
  ...
  ipbox(0,mi,mjp,nmp,lbi,lbj,lbjr,lbjp,lev,-2,npb,tfmm);
...
// Step 3. M2M-translation @ lev lmax-1 to 2
    ...
    ipbox(0,mi,mj,nmp,lbi,lbj,lbjr,lbjp,lev,0,npb,tfmm);
...
// Step 6. M2L-translation @ sublev npb
    ...
    if( neq == 0 ) {
      bsm2p(nmp,mp,lbi,lbjp,lev,npb,rb);
    } else if( 1 <= neq && neq <= 3 ) {
      stm2p(nmp,mp,lbi,lbjp,lev,npb,rb);
    } else {
      potm2p(nmp,mp,lbi,lbjp,lev,npb,rb);
    }
...
// Step 7. M2P-translation @ sublev npb-1 to 1
    ...
    for( ipb=npb-1; ipb>=1; ipb-- ) {
      ...
      if( neq == 0 ) {
        bsm2p(nmp,mp,lbi,lbjp,lev,ipb,rb);
      } else if( 1 <= neq && neq <= 3 ) {
        stm2p(nmp,mp,lbi,lbjp,lev,ipb,rb);
      } else {
        potm2p(nmp,mp,lbi,lbjp,lev,ipb,rb);
      }
...
// Step 8. M2P-translation @ sublev 1 to lev 1
...
// Step 9. M2P-translation @ lev 2 to lmax
    ...
    if( lmax > lmin ) {
      for( lev=lmin+1; lev<=lmax; lev++ ) {
        ...
        ipbox(0,mi,mj,nmp,lbi,lbj,lbjr,lbjp,lev,ipb,npb,tfmm);

        toc = tic;
        tic = get_time();

        if( neq == 0 ) {
          bsm2p(nmp,mp,lbi,lbjp,lev,0,rb);
        } else if( 1 <= neq && neq <= 3 ) {
          stm2p(nmp,mp,lbi,lbjp,lev,0,rb);
        } else {
          potm2p(nmp,mp,lbi,lbjp,lev,0,rb);
        }
