...
extern int *nei,*nfi,**ndj,*nej,*nfj,*nlbj,**ndl,*nel,*nlbl,**ndm,*nem,*nlbm,*njb,**nsij,**npx;
...
void isbox(int n0, int n1, int mj, int& mjp, int nmp, int lbi, int lbj, int lbl, int lbm, int& lbjr, int& lbjrp, int lev, int ipb, int npb, int jlm, double* tfmm) {
  int nnmax,n3max,nmmax,ii,i,lbjp,lblr,lbmr,k,j,nr,ic,jj,np;
  double tic,toc,rb;
  ...
  if( lev == 1 ) {
    mpisendm2l(nmp,jlm,lbj,lbjrp,lev,tic,tfmm);
    tic = get_time();
  } else {
...
// nej
    ...
    lbjrp = lbjp;
    for( ii=0; ii<nr; ii++ ) {
      if( nej[nfj[lbjrp]] == -1 && npx[2][lbjrp] == 0 ) {
        nej[nfj[lbjrp]] = lbjr;
        lbjr++;
      }
      lbjrp++;
...
// nel
    ...
    lbjrp = lbjp;
    for( ii=0; ii<nr; ii++ ) {
      if( nel[nfj[lbjrp]] == -1 && npx[2][lbjrp] > 0 ) {
        nel[nfj[lbjrp]] = lblr;
        lblr++;
      }
      lbjrp++;
...
// nem
    ...
    lbjrp = lbjp;
    for( ii=0; ii<nr; ii++ ) {
      if( nem[nfj[lbjrp]] == -1 && npx[2][lbjrp] < 0 ) {
        nem[nfj[lbjrp]] = lbmr;
        lbmr++;
      }
      lbjrp++;
