  ...
  for( ii=0; ii<lbi; ii++ ) {
    for( i=ndi[0][ii]; i<=ndi[1][ii]; i++ ) {
      for( ij=0; ij<nij[ii]; ij++ ) {
        ...
        gxd[i] -= 0.25/pi*(gxi[i]*gxx+gyi[i]*0.5*(gxy+gyx)+gzi[i]*0.5*(gxz+gzx));
        gyd[i] -= 0.25/pi*(gxi[i]*0.5*(gyx+gxy)+gyi[i]*gyy+gzi[i]*0.5*(gyz+gzy));
        gzd[i] -= 0.25/pi*(gxi[i]*0.5*(gzx+gxz)+gyi[i]*0.5*(gzy+gyz)+gzi[i]*gzz);
      }
    }
  }
}
